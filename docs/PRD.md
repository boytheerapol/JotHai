...
LIFF dashboard (เปิดในตัว LINE) แสดงรายงาน 4 แท็บ (ภาพรวม / หมวดหมู่ / เทียบเดือน / รายการ) พร้อม chart + รายการแก้ไขได้
...
- **`Code.gs`** (Backend Router) — `doGet(e)` ทำหน้าที่เป็น Data API ส่ง JSON (`?api=overview`, `?api=list`, `?api=trend`); `doPost(e)` รับ LINE Webhook และ LIFF Write POST (edit/delete/undo ผ่าน idToken)
...
- **Frontend (GitHub Pages)** — split ES-module structure (no build step):
  - `index.html` — shell: global month-nav header, 4-way tab bar, 4 `<section>` view containers; CDN scripts: liff / Chart.js / **chartjs-plugin-datalabels@2** / SweetAlert2
  - `css/styles.css` — design-system tokens (§2–§6) + all component CSS; รวม `.spinner` + `@keyframes spin` + reduced-motion guard สำหรับ loading indicator; `.summary-strip.loading` (opacity 0.4 ขณะ fetch); `.day-subtotal` ขนาด `--text-heading` (18px) เพื่อโดดเด่นกว่ารายการ (14px)
  ...
  - `js/views/overview.js`, `categories.js`, `trend.js`, `entries.js` — one module per tab
  - `js/app.js` — `liff.init()`, header wiring, tab router, data loading, `actions` object; `renderMonthLabel()` แยกออกมาเพื่อ flip label ก่อน fetch; error path เคลียร์ totals เป็น ฿0
...
- `?api=overview` → ส่งคืน `incomeTotal`, `expenseTotal`, `incomesByCategory`, `expensesByCategory`, `incomesByHashtag`, `expensesByHashtag` สำหรับแท็บ ภาพรวม + หมวดหมู่
- `?api=list` → ส่งคืนรายการ active ในเดือนนั้น พร้อมแนบ `categories` จาก Sheet กลับมาสร้าง Dropdown
- `?api=trend` → ส่งคืน array 6 รายการ `{month, incomeTotal, expenseTotal}` สำหรับแท็บ เทียบเดือน
...
- **Decoupled Architecture (shipped):** LIFF frontend รันบน GitHub Pages แล้ว — ปลดล็อก Iframe Sandbox ได้สมบูรณ์ โหลดเร็วขึ้นและปรับแต่ง UI ได้เต็มที่ด้วย native ES modules ไม่มี build step Backend ทั้งหมดยังเป็น GAS 100%
